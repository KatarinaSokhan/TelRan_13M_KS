...
public class Forum {
    ...
    public Post[] getNotNullablePosts() {
        Post[] notNullablePosts = new Post[size];
       // int index = 0;

        for (int i = 0; i < size; i++) {
           notNullablePosts[i] = posts[i];
           /* notNullablePosts[index] = posts[i];
            index++;*/
        }
        return notNullablePosts;
    }
    ...
    public void removePost(Post postToDelete, Scanner scanner) {
        if (checkPassword(scanner)) {
            boolean flag = false;
            Post[] newPosts = new Post[posts.length];
            int index = 0;

            for (int i = 0; i < size; i++) {
                if (posts[i].getPostID() != postToDelete.getPostID()) {
